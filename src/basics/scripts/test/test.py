...
###############################################################################################################################################################
    ...
    def timerCB(self, data):
        ...
        if self.obj_count >= 1 and (self.remapped_ranges[180] or self.remapped_ranges[195] or self.remapped_ranges[120]) < 1.5 :
            print("라이다모드")
            self.control_B()
        else:
            print("카메라모드")
    ...
    def cam_drive(self):
        ...
        if self.stop_flag == 0:
            ...
            left_steer_angle = self.cal_turnleft_steer()
            self.publish(left_steer_angle, 1000)
        elif self.stop_flag == 1:
            if self.len_left > 150 and self.len_right > 150:
                self.go_straight()
            elif self.line_width is not None and self.line_width > 360:
                self.entry_rotate()
            else:
                steer_angle = self.cal_steer()
    ...
    def stop_line(self, l):
        ...
        if stop_line_status and not self.stopline_detected:
            ...
            pass
    # stop flag 3일때 좌회전 조향각 계산
    def cal_turnleft_steer(self):
